  ...
  render() {
    ...
    return (
      <div className="news-post">         
          <p>{text}</p>
          <input onChange={this.handleChange} onKeyDown={this.handleKeyDown} value={commentInput} className="comment-input"/>
          <div>
            {comments.map(comment => (
              <Comment key={comment.id} id={comment.id} text={comment.text} onDelete={this.handleDelete}/>
            ))}
          </div>
      </div>
    );
